...
const GuestCard: Component<GuestCardProps> = (props) => {
  ...
  return (
    <div
      class={`group bg-white/80 backdrop-blur-sm border rounded-xl mobile-px mobile-py hover:shadow-xl transition-all duration-500 card-hover ${cardBorderColor()}`}
    >
      {/* Header */}
      <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 space-y-3 sm:space-y-0">
        <div class="flex-1">
          <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 mb-2 space-y-2 sm:space-y-0">
            <h3 class="text-lg sm:text-xl font-medium text-gray-900 leading-tight">
              {props.guest.name}
            </h3>
            <div
              class={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border self-start ${
          ...
          <div class="space-y-2">
            {props.guest.email && (
              <div class="flex items-center text-sm text-gray-600">
                ...
                <span class="font-light break-all">{props.guest.email}</span>
              </div>
            )}
        ...
        <div class="flex items-center space-x-2 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
          <button
            onClick={() => props.onEdit(props.guest)}
            class="btn-mobile p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded-lg transition-all duration-300 focus-mobile"
            title="Edit guest"
          >
            <svg
              class="w-5 h-5"
          ...
          <button
            ...
            class="btn-mobile p-2 text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition-all duration-300 focus-mobile"
            title="Delete guest"
          >
            <svg
              class="w-5 h-5"
        ...
        {props.guest.meal_preference && (
          <div class="flex items-center p-3 bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg border border-orange-100">
            ...
            <div class="min-w-0">
              ...
              <div class="text-sm text-gray-700 font-light truncate">
                {props.guest.meal_preference}
              </div>
            </div>
          </div>
      ...
      {props.guest.notes && (
        <div class="p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-100 mb-4">
          <div class="flex items-start space-x-3">
            <div class="w-6 h-6 bg-gray-100 rounded-lg flex items-center justify-center mt-0.5 flex-shrink-0">
              <svg
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-sm text-gray-700 font-light break-words">
                {props.guest.notes}
              </div>
            </div>
          </div>
              ...
              {(plusOne, index) => (
                <div class="bg-gradient-to-r from-purple-50/50 to-violet-50/50 p-4 rounded-lg border border-purple-100/50">
                  <div class="flex items-start justify-between">
                    <div class="flex-1 min-w-0">
                      <div class="font-medium text-gray-800 text-sm mb-1 truncate">
                        {plusOne.name ||
                      ...
                      <div class="space-y-1">
                        {plusOne.meal_preference && (
                          <div class="flex items-center text-xs text-gray-600">
                            ...
                            <span class="font-light break-words">
